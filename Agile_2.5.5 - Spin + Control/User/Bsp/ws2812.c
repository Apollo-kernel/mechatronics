...
/*
 * STM32H723 + SPI6 often uses BDMA (D3 domain). Reserve a small chunk in RAM_D3
 * for the DMA buffer so the WS2812 update can run without extra linker changes.
 */
...
static volatile uint8_t ws_busy = 0u;
...
static inline void ws2812_encode_one(uint8_t r, uint8_t g, uint8_t b)
{
    ...
    for (i = 0; i < 8; i++)
    {
        data_buf[7  - i] = ((g >> i) & 0x01u) ? WS2812_SYM_1 : WS2812_SYM_0;
        data_buf[15 - i] = ((r >> i) & 0x01u) ? WS2812_SYM_1 : WS2812_SYM_0;
        data_buf[23 - i] = ((b >> i) & 0x01u) ? WS2812_SYM_1 : WS2812_SYM_0;
    }
}
...
void WS2812_Ctrl(uint8_t r, uint8_t g, uint8_t b)
{
    ...
    st = HAL_SPI_Transmit_DMA(&WS2812_SPI_UNIT, ws_buf, WS_BUF_LEN);
    if (st != HAL_OK)
    {
        ws_busy = 0u;
    }
