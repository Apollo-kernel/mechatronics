...
#define PITCH_ZERO_DEG      -0.7f
#define PITCH_ZERO_RAD      (PITCH_ZERO_DEG * 0.01745329252f)
 ...
 *
 * Default -25 rpm keeps the current backward-25 behavior.
 */
...
#define TURN_OUT_LIMIT             9000.0f

#define YAW_REF_RATE_DPS_DEFAULT   0.0f
#define YAW_REF_RATE_DPS_LIMIT     180.0f
#define DEG_TO_RAD_LOCAL           0.01745329252f
#define YAW_REF_DT_MAX_S           0.05f      /* avoid big jump after pause */

#define JOYSTICK_P15_UPDATE_EPS_DPS 0.01f
...
/*
 * P8 runtime speed target.
 * Unit: rpm.
 * Positive = forward, negative = backward.
 */
static volatile float g_speed_target_rpm = SPEED_TARGET_DEFAULT_RPM;
...
/*
 * P9:
 * 0 = normal position hold / return mode
 * 1 = keep refreshing position reference to current position
 */
static volatile uint8_t g_chassis_pos_ref_follow_enable = 1u;
...
static uint32_t yaw_ref_last_tick = 0u;
static volatile float g_yaw_ref_rate_dps = YAW_REF_RATE_DPS_DEFAULT;

/* Website / ESP32 joystick combined command: #P15=<deg/s>,P8=<rpm>! */
...
int Balance_ParamSetById(uint8_t id, float value)
{
    switch (id)
    {
        ...
        case 15:
            if ((value >= -YAW_REF_RATE_DPS_LIMIT) && (value <= YAW_REF_RATE_DPS_LIMIT))
            {
                g_yaw_ref_rate_dps = value;

                /*
                 * #P15=0! 时重新抓取当前 yaw 作为保持目标，
                 * 避免停止旋转后又回到旧的 yaw_ref_total。
                 */
                if (fabsf(value) < 0.001f)
                {
                    yaw_ref_valid = 0u;
                    yaw_ref_last_tick = 0u;
                }
...
static void balance_cli_print_help(void)
{
    ...
    (void)UART1_LogPrintfDrop("#P15=-180..180!        : yaw target rate, deg/s; 0=hold current yaw\r\n");
    (void)UART1_LogPrintfDrop("#P15=<raw>,P8=<raw>!   : joystick combo, raw +/-1000 -> P15 +/-180 dps, P8 +/-60 rpm\r\n");
...
static void balance_cli_print_status(void)
{
    ...
    (void)UART1_LogPrintfDrop(
        "P10 led=%s P11 R=%u P12 G=%u P13 B=%u\r\n",
    ...
    (void)UART1_LogPrintfDrop(
    "P15 yaw_rate=%.3f deg/s yaw_ref=%.2fdeg\r\n",
    g_yaw_ref_rate_dps,
    yaw_ref_total * 57.2957795f);
...
static uint8_t balance_handle_hash_command(const char *line)
{
    ...
    /* #VOFA! or #STREAM! : switch to VOFA mode */
...
uint8_t Balance_HandleJoystickCommand(const char *line)
{
    float p15_raw;
    float p8_raw;
    float p15_dps;
    ...
    /*
     * Website / ESP32 / VOFA joystick combo frame:
     ...
     *
     * Examples:
     *   #P15=0,P8=0!          -> P15=0 deg/s,    P8=0 rpm
     *   #P15=1000,P8=0!       -> P15=180 deg/s,  P8=0 rpm
     *   #P15=-1000,P8=1000!   -> P15=-180 deg/s, P8=60 rpm
     *   #P15=500,P8=-500!     -> P15=90 deg/s,   P8=-30 rpm
     */
    if (sscanf(line, "#P15=%f , P8=%f !", &p15_raw, &p8_raw) != 2)
    {
        return 0u;
    }

    /* Clamp raw joystick input first. */
    p15_raw = clampf_local(p15_raw, -1000.0f, 1000.0f);
    p8_raw  = clampf_local(p8_raw,  -1000.0f, 1000.0f);

    /*
     * Map joystick raw value to real control parameter.
     *
     * P15: +/-1000 -> +/-180 deg/s
     * P8 : +/-1000 -> +/-60 rpm
     */
    p15_dps = p15_raw * 180.0f / 1000.0f;
    p8_rpm  = p8_raw  * 60.0f  / 1000.0f;

    /* Forward / backward joystick -> P8 speed target. */
    (void)Balance_ParamSetById(8u, p8_rpm);

    /*
     * Left / right joystick -> P15 yaw target rate.
     *
     * Important:
     * Do not repeatedly call P15=0 on every #P15=0,P8=...! frame.
     * Otherwise yaw_ref_valid will be cleared every frame and yaw hold becomes weak.
     */
    if (fabsf(p15_dps - g_yaw_ref_rate_dps) > JOYSTICK_P15_UPDATE_EPS_DPS)
    {
        (void)Balance_ParamSetById(15u, p15_dps);
    }
...
void Balance_CLI_ProcessLine(char *line)
{
    ...
    /*
     * Website / ESP32 joystick combo frame:
     *   #P15=<yaw_rate_dps>,P8=<speed_rpm>!
     *
     * No UART print here, because the website/ESP32 may send this periodically.
     */
    if (Balance_HandleJoystickCommand(line))
    {
        return;
    }

    /* first handle shared hash commands: #P..., #BAL..., #CLI!... */
...
static float speed_loop_calc(float speed_d_raw_20ms, uint8_t reset)
{
    ...
    float speed_target_rpm = g_speed_target_rpm;
    float speed_target_d_raw_20ms = SPEED_TARGET_RPM_TO_D_RAW_20MS(speed_target_rpm);
...
static float turn_loop_calc(float yaw_total, float gyro_z, uint8_t reset)
{
    ...
    if (!yaw_ref_valid)
    {
        ...
        yaw_ref_last_tick = now;
        yaw_ref_valid = 1u;
        return 0.0f;
    }

    dt_s = ((float)(now - yaw_ref_last_tick)) * 0.001f;
    yaw_ref_last_tick = now;

    if (dt_s > YAW_REF_DT_MAX_S)
    {
        dt_s = YAW_REF_DT_MAX_S;
    }

    /*
     * Positive g_yaw_ref_rate_dps means:
     * yaw_ref_total increases continuously.
     *
     * Default:
     * 10 deg/s -> 0.1 deg every 10 ms.
     */
    yaw_ref_total -= g_yaw_ref_rate_dps * DEG_TO_RAD_LOCAL * dt_s;
...
static void balance_motor_init(void) {
    ...
    // WS2812_Ctrl(0, 0, 90);
    WS2812_Ctrl(g_ws2812_r, g_ws2812_g, g_ws2812_b);
    osDelay(3000);
        ...
        if (Balance_GetClosedLoopEnable() == 0U)
{
    ...
    continue;
        ...
        pitch = INS.Pitch - PITCH_ZERO_RAD;
        pitch_gyro = INS.Gyro[0];
