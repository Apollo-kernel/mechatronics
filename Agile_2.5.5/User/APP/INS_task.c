...
static void INS_PrintYPR_USART1_DMA(void)
{
	...
	(void)UART1_LogPrintfDrop("Yaw=%.2fdeg Pitch=%.2fdeg Roll=%.2fdeg\r\n",
							  INS.Yaw   * RAD_TO_DEG,
...
{
	UART1_LogTxCpltISR(huart);
	Balance_MotorUartTxDone(huart);
}
...
{
	VOFA_UART1_RxEvent(huart, Size);

	Balance_MotorUartRxEvent(huart, Size);
}