...
#define SPEED_LPF_ALPHA            0.92f         //0.97  //0.92
...
static void balance_cli_print_status(void)
{
    ...
    (void)UART1_LogPrintfDrop(
        "P1 bal_kp=%.3f P2 bal_kd=%.3f P3 spd_kp=%.3f P4 spd_ki=%.3f P5 turn_kp=%.3f P6 turn_kd=%.3f\r\n",
...
static float speed_loop_calc(float speed_d_raw_20ms, uint8_t reset)
{
    ...
    err = SPEED_TARGET_D_RAW_20MS - speed_lpf_d_raw_20ms;

    speed_i_term += err;
    speed_i_term = clampf_local(speed_i_term, -SPEED_I_LIMIT, SPEED_I_LIMIT);

    out = g_balance_param.speed_kp * err + g_balance_param.speed_ki * speed_i_term;
