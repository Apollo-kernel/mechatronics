...
float target_speed_x = 15.0f;  // Forward/Backward speed in RPM (e.g., 10 for forward, -10 for backward)
...
static float speed_loop_calc(float speed_d_raw_20ms, uint8_t reset, float speed_target_cmd)
{
    ...
    speed_err = speed_target_cmd - speed_lpf_d_raw_20ms;
        ...
        if (Balance_GetClosedLoopEnable() == 0U)
{
    ...
    continue;
        ...
        /* --- Motion sequencing: hold translation until the turn is complete --- */
        ...
        {
            // After the rotation is finished, apply the user-selected straight speed.
